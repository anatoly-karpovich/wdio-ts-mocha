 ...
 * @param {AsyncCallback<T>} callback A function that accepts up to three arguments. The filter method calls the predicate function one time for each element in the array. Should returns `boolean` result. 
 * @returns {Promise<Array<T>>} filtered array 
 */ 
export default async function filter<T>(array: MaybePromise<readonly T[]>, callback: AsyncCallback<T>): Promise<T[]> { 
